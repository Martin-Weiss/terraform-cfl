...
resource "null_resource" "kubeconfig" {
  provisioner "local-exec" {
    command = "while true; do if curl -k --max-time 5 https://${data.equinix_metal_device.seed_device.access_public_ipv4}/version|grep gitVersion; then echo completed; rsync -e 'ssh -o StrictHostKeyChecking=no' --rsync-path=\"sudo rsync\" rancher@${data.equinix_metal_device.seed_device.access_public_ipv4}:/etc/rancher/rke2/rke2.yaml ../harvester-rke2.yaml && sed -i \"s#127.0.0.1#${data.equinix_metal_device.seed_device.access_public_ipv4}#g\" ../harvester-rke2.yaml; exit 0; fi; sleep 5; done"
  }
}
